...
const AddPoliticianForm = ({ open, onOpenChange }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await addPolitician(submitData);
      
      if (response.success) {
        toast({
          title: "Success",
          description: "Politician added successfully",
        });
      ...
      toast({
        title: "Error",
        description: "Failed to add politician",
        variant: "destructive",
      });
    } finally {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="bg-got-darkgray border border-got-gold text-got-ivory max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-xl font-cinzel text-got-gold">Add New House</DialogTitle>
          <DialogDescription className="text-got-ivory/80">
            Enter the details for the new great house of the realm.
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 py-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="Party" className="text-got-gold">Party</Label>
            <Input 
              id="Party" 
              name="Party" 
